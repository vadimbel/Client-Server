...
uint32_t calculate_checksum(const std::vector<unsigned char>& binaryContent) {
    ...
    std::string filename = "tempfile_" + std::to_string(now) + ".tmp";
    auto temp_file_path = std::filesystem::temp_directory_path() / filename;
    ...
    // Write binary content to the temporary file
    {
        ...
        temp_file.write(reinterpret_cast<const char*>(binaryContent.data()), binaryContent.size());
    }
    ...
    uint32_t checksum = 0;
    {
        ...
        std::vector<char> buffer(std::istreambuf_iterator<char>(temp_file), {});
        checksum = static_cast<uint32_t>(memcrc(buffer.data(), buffer.size()));
    }
